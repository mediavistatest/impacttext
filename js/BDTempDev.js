...
var ngInbox = {
    _internal : {
        ...
        Methods : {
            ...
            OutboundRestore : function(controllerParent, messageToChangeStatusArray, changeToStatus, callback) {
                var params = {
                    apikey : controllerParent.$cookieStore.get('inspinia_auth_token'),
                    accountID : controllerParent.$cookieStore.get('inspinia_account_id')
                };
            ...
            RestoreToInboxMessage : function(controllerParent, messageList) {
                ...
                case 'messages_inbound' :
                    ...
                    break;
                case 'messages_outbound' :
                    ngInbox._internal.Methods.OutboundRestore(controllerParent, messageList, controllerParent.restoreMessageStatus, callback);
                    break;
                }
            },
        ...
        ngInboxNotifyCtrl : function($scope, notify) {
            ...
            $scope.RestoreToInboxMsg = function() {
                notify({
                    message : 'Your message(s) has been restored!',
                    classes : 'alert-success'
                });
            };
        ...
        PopulateSend : function($sendScope) {
            try {
                continueFunction = function() {
                    ...
                    scheduledDateTime.SetDate = new Date($sendScope.controllerParent.clickedMessage.scheduledDate.substring(0, 10) + 'T00:00:00');
        ...
        PopulateSend : function($sendScope) {
            try {
                ...
                continueFunction = function() {
                    ...
                    //                    scheduledDateTime.SetDate = new Date(ngFunctions.ConvertDateToYYYYmmDD($sendScope.controllerParent.clickedMessage.scheduledDate.substring(0, 10), 'YYYY-DD-mm') + 'T00:00:00');
                    ...
                    if ($sendScope.controllerParent.clickedMessage.recurringend != null) {
                        scheduledDateTime.SetRecurringEndDate = new Date($sendScope.controllerParent.clickedMessage.recurringend.substring(0, 10) + 'T00:00:00');
                    } else {
        ...
        Events : {
            Save_onClick : function(cpo) {
                ngSettings.NumberNames.ServerRequests.SetDefault(cpo, function() {
                    ...
                    cpo.$scope.$broadcast('itMessage', {
                        message : 'Default CentralMessage Number succesfully set.'
                    });
                }, function() {
                    cpo.$scope.$broadcast('itError', {
                        message : 'Failed to set the default CentralMessage Number!'
                    });
                });
                    ...
                    if (keyword_update_count > 0) {
                        for (var idx in keyword_update) {
                            ...
                            function(data, keyword_id) {
                                ...
                                var message = (errorMessageParts.length > 1) ? $.trim(errorMessageParts[1]) : 'Failed to update keyword';
                ...
                waitForKeywordUpdate.promise.then(function() {
                    ...
                    if (cpo.$scope.main.accountInfo.companyID) {
                        ...
                        cpo.$scope.main.ServerRequests.accountKeywordGet(function() {
                            ...
                            function(data) {
                                ...
                                cpo.$scope.$broadcast('itError', {
                                    message : 'Failed to get CentralMessage Number!'
                                });
                ...
                if (cpo.remainingSaveCount == 0) {
                    ...
                    if (cpo.saveErrorCount > 0) {
                        if (cpo.saveSuccessCount > 0) {
                            cpo.$scope.$broadcast('itError', {
                                message : 'Failed to save CentralMessage Number settings for some numbers (' + cpo.saveErrorCount + ')!'
                            });
                        } else {
                            cpo.$scope.$broadcast('itError', {
                                message : 'Failed to save CentralMessage Number settings!'
                            });
                        }
                    } else {
                        cpo.$scope.$broadcast('itMessage', {
                            message : 'CentralMessage Number settings saved'
                        });
                    }
                    // Reset counters
    ...
    Autoresponder : {
        _internal : {
            ...
            ResetRules : function(cpo) {
                ...
                cpo.AddToListKeywordActionRule = {
                    checked : false,
                    name : 'mvAddToList',
            ...
            fillAddToListRule : function(cpo, inAction) {
                //rule = inAction;
                cpo.AddToListKeywordActionRule.checked = (inAction.status == 'A');
                cpo.AddToListKeywordActionRule.autoResponderKeywordActionID = inAction.autoResponderKeywordActionID;

                var inParams = JSON.parse(inAction.parameters);

                if (inParams.contactlistid) {
                    ngSettings.Autoresponder._internal.SelectList(cpo, inParams.contactlistid);
                }
            },
            SelectList : function(cpo, contactListId) {
                if (cpo.$scope.main.contactLists) {
                    if (contactListId) {
                        for (var i = 0; i < cpo.$scope.main.contactLists.length; i++) {
                            if (cpo.$scope.main.contactLists[i].contactListID == String(contactListId)) {
                                cpo.AddToListKeywordActionRule.selectedList = cpo.$scope.main.contactLists[i];
                                break;
                            }
                        }
                    }
                    ...
                    setTimeout(function() {
                        ngSettings.Autoresponder._internal.SelectList(cpo, contactListId);
                    }, 200);
                }
            }
        },
        ServerRequests : {
            AddAutoresponder : function(cpo, callback) {
                if (cpo.$scope.main.accountInfo.companyID) {
                    var params = {
                        ...
                        // startdate : cpo.arCtrl.validFrom.toISOString().substring(0, 10) + 'T00:00:00',
                        // ,sethttp : 1
                    };
                    ...
                    .error(function(data, status, headers, config) {
                        ...
                        console.log('autoresponder_add: ' + data.apitext);
                    });
                } else {
                    setTimeout(function() {
                    ...
                    if (result.apidata.length > 0) {
                        for (var i = 0; i < result.apidata.length; i++) {
                            ...
                            case 'mvDelay2':
                                ...
                                break;
                            case 'mvAddToList':
                                ngSettings.Autoresponder._internal.fillAddToListRule(cpo, result.apidata[i]);
                                break;
                            default:
                                break;
                            }
            ...
            AddKeyword : function(cpo, callback) {
                ...
                if (cpo.clickedKeyword.startDate) {
                    params.startdate = cpo.clickedKeyword.startDate;
                }
            ...
            AddKeywordCallback : function(cpo, result) {
                if (result.apicode == 0) {
                    ...
                    ngSettings.Autoresponder.ServerRequests.ResolveActions(cpo);
                } else {
                    cpo.$scope.$broadcast('itError', {
                        message : 'Error! ' + result.apitext
                    });
            ...
            ResolveActions : function(cpo, actionList) {
                for (var i = 0; i < cpo.keywordActionRules.length; i++) {
                    if (cpo.keywordActionRules[i].autoResponderKeywordActionID) {
                        ngSettings.Autoresponder.ServerRequests.ModifyActions(cpo, [cpo.keywordActionRules[i]], ngSettings.Autoresponder.ServerRequests.ModifyActionsCallback);
                    } else {
                        ngSettings.Autoresponder.ServerRequests.AddActions(cpo, [cpo.keywordActionRules[i]], ngSettings.Autoresponder.ServerRequests.AddActionsCallback);
                    }
                }
                if (cpo.AddToListKeywordActionRule.autoResponderKeywordActionID) {
                    ngSettings.Autoresponder.ServerRequests.ModifyActions(cpo, [cpo.AddToListKeywordActionRule], ngSettings.Autoresponder.ServerRequests.ModifyActionsCallback);
                } else {
                    ngSettings.Autoresponder.ServerRequests.AddActions(cpo, [cpo.AddToListKeywordActionRule], ngSettings.Autoresponder.ServerRequests.AddActionsCallback);
                }
            },
        ...
        Events : {
            ...
            Save_onClick : function(cpo) {
                ...
                if (cpo.arCtrl.validFrom) {
                    // cpo.clickedKeyword.startDate = cpo.arCtrl.validFrom.toISOString().substring(0, 10) + 'T00:00:00';
                ...
                if (cpo.arCtrl.validUntil) {
                    // cpo.clickedKeyword.endDate = cpo.arCtrl.validUntil.toISOString().substring(0, 10) + 'T00:00:00';
        ...
        PopulateRules : function(cpo) {
            ...
            if (cpo.clickedKeyword.startDate) {
                cpo.arCtrl.validFrom = new Date(ngFunctions.ConvertDateToYYYYmmDD(cpo.clickedKeyword.startDate.substring(0, 10), 'YYYY-DD-mm') + 'T00:00:00');
            ...
            if (cpo.clickedKeyword.endDate) {
                cpo.arCtrl.validUntil = new Date(ngFunctions.ConvertDateToYYYYmmDD(cpo.clickedKeyword.endDate.substring(0, 10), 'YYYY-DD-mm') + 'T00:00:00');
    ...
    Scheduled : {
        ...
        ServerRequests : {
            ...
            GetReportCallback : function(cpo, result) {
                ...
                setTimeout(function() {
                    if (cpo.rsCtrl.ngReportsListOptions.selectAll) {
        ...
        ClearView : function(cpo) {
            ...
            cpo.rsCtrl.selectedContactGroup = {
                id : null,
                name : 'All lists',
                group : 'Lists'
            };
        ...
        PopulateView : function(cpo) {
            if (cpo.clickedReport.parameters.startdate) {
                cpo.rsCtrl.StartDate = new Date(ngFunctions.ConvertDateToYYYYmmDD(cpo.clickedReport.parameters.startdate.substring(0, 10), 'YYYY-DD-mm') + 'T00:00:00');
            }
            if (cpo.clickedReport.parameters.enddate) {
                cpo.rsCtrl.EndDate = new Date(ngFunctions.ConvertDateToYYYYmmDD(cpo.clickedReport.parameters.enddate.substring(0, 10), 'YYYY-DD-mm') + 'T00:00:00');
            }
            ...
            if (cpo.clickedReport.parameters.reportdate) {
                cpo.rsCtrl.sheduledDateTime = new Date(cpo.clickedReport.parameters.reportdate.substring(0, 10) + 'T00:00:00');
            ...
            if (cpo.clickedReport.parameters.contactlistid) {
                for (var group in cpo.$scope.main.contactGroups) {
                    if (cpo.$scope.main.contactGroups[group].group == 'Lists' && cpo.$scope.main.contactGroups[group].id == cpo.clickedReport.parameters.contactlistid) {
...
var ngAccount = {
    ServerRequests : {
        CreateToken : function(cpo, callback) {
            if (cpo.$scope.main.accountInfo.companyID) {
                var params = {
                    apikey : cpo.$scope.main.authToken,
                    externalaccountnumber : cpo.$scope.main.accountInfo.externalAccountNumber
                };
                var $param = $.param(params);

                //POST
                cpo.$http.post(inspiniaNS.wsUrl + 'createtoken', $param)
                // success function
                .success(function(result) {
                    if (result.apicode == 0) {
                        // cpo.$scope.$broadcast('itMessage', {
                        ...
                        // cpo.$scope.$broadcast('itError', {
                        // message : 'createtoken: ' + result.apitext
                        // });
                    }
                })
                // error function
                ...
                setTimeout(function() {
                    ngAccount.ServerRequests.CreateToken(cpo, callback);
                }, 500);
            }
        },
